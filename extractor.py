...
def clean_visual_column_name(name: str) -> str:
    ...
    name = name.replace("[", "").replace("]", "")
    name = re.sub(
        r"^(none|sum|avg|min|max|count|attr|yr|mn|dy|qd|tdc|usr|pcto|rank):",
        "",
        name,
        flags=re.IGNORECASE,
    )
...
def extract_calculation_id(raw: str) -> str:
    """Pull Calculation_XXXX from Tableau column refs / federated names."""
    if not raw:
        return ""
    s = str(raw).replace("[", "").replace("]", "")
    m = re.search(r"(Calculation_\d+)", s, flags=re.IGNORECASE)
    return m.group(1) if m else ""


def resolve_calc_meta(
    raw_ref: str,
    clean_col: str,
    calc_lookup: Dict[str, Any],
    calc_name_lookup: Dict[str, Any],
):
    """Resolve a Tableau column ref to a calculations[] entry."""
    calc_id = extract_calculation_id(raw_ref) or extract_calculation_id(clean_col)
    return (
        calc_lookup.get(clean(raw_ref))
        or calc_lookup.get(calc_id)
        or calc_name_lookup.get(clean_col)
        or calc_name_lookup.get(clean(raw_ref))
    )
                    ...
                    for table in conn.catalog.get_table_names(schema):
                        ...
                        try:
                            ...
                            for c in table_def.columns:
                                ...
                                if "int" in col_type:
                                    mapped_type = "integer"
                                elif "double" in col_type or "numeric" in col_type or "float" in col_type:
                                    mapped_type = "real"
...
def extract_visual_metadata(
    ...
    worksheets_data = []
    dashboards_data = []
    calc_lookup = {c["calculationId"]: c for c in calculations_list if c.get("calculationId")}
    calc_name_lookup = {c["name"]: c for c in calculations_list if c.get("name")}
    ...
    for ws in root.findall(".//worksheet"):
        ...
        for dep in ws.findall(".//datasource-dependencies"):
            for col_inst in dep.findall("column-instance"):
                ...
                calc_meta = resolve_calc_meta(raw_col, clean_col, calc_lookup, calc_name_lookup)
                if calc_meta is None:
                    calc_meta = resolve_calc_meta(inst_name, clean_col, calc_lookup, calc_name_lookup)
                is_calc = calc_meta is not None
                ...
                shelf_location = "Marks"
                if inst_name in rows_str:
                    shelf_location = "Rows"
                elif inst_name in cols_str:
                    shelf_location = "Columns"

                # Bind name = caption for calcs (never leave Calculation_* as column)
                display_name = calc_meta["name"] if is_calc else clean_col
        ...
        # Dedupe fields (same calc/column on same shelf — e.g. pcto vs usr)
        deduped_fields = []
        seen_field_keys = set()
        for f in fields:
            key = (
                f.get("calculationId") or f.get("name") or f.get("column"),
                f.get("shelf"),
                f.get("role"),
            )
            ...
            columns_ref.append(
                {
                    ...
                    "calculationId": f.get("calculationId"),
                }
            )
        ...
        for pane in ws.findall(".//pane"):
            for role_tag in ["color", "size", "tooltip", "text", "lod", "wedge-size"]:
                for el in pane.findall(f".//{role_tag}"):
                    col_ref = el.get("column", "")
                    clean_c = clean_visual_column_name(col_ref)
                    calc_m = resolve_calc_meta(col_ref, clean_c, calc_lookup, calc_name_lookup)

                    display = calc_m["name"] if calc_m else clean_c
                    enc_obj = {
                        "name": display,
                        "role": role_tag,
                        "encoding": role_tag,
        ...
        # Dedupe encodings by role + resolved name
        deduped_enc = []
        seen_enc = set()
        for e in encodings:
            key = (e.get("role"), e.get("name"), e.get("calculationId"))
            if key in seen_enc:
                continue
            seen_enc.add(key)
            deduped_enc.append(e)
        encodings = deduped_enc

        # 3. Filters
        for flt in ws.findall(".//filter"):
            flt_col = flt.get("column", "")
            clean_flt = clean_visual_column_name(flt_col)
            calc_m = resolve_calc_meta(flt_col, clean_flt, calc_lookup, calc_name_lookup)

            display = calc_m["name"] if calc_m else clean_flt
            flt_obj = {
                "name": display,
                "column": display,
                "fieldType": "calculatedField" if calc_m else "column",
            ...
            if calc_m:
                flt_obj["calculationId"] = calc_m["calculationId"]
            filters.append(flt_obj)

        # Dedupe filters
        deduped_flt = []
        seen_flt = set()
        for f in filters:
            key = (f.get("name"), f.get("calculationId"), f.get("filterClass"))
            if key in seen_flt:
                continue
            seen_flt.add(key)
            deduped_flt.append(f)
        filters = deduped_flt
    ...
    for db in root.findall(".//dashboard"):
        ...
        # Unique worksheet names, preserve order
        seen_ws = set()
        ws_list_unique = []
        for z_name in ws_list:
            if z_name not in seen_ws:
                seen_ws.add(z_name)
                ws_list_unique.append(z_name)

        # Unique visuals by name + rounded pixel box
        seen_vis = set()
        visuals_unique = []
        for vis in visuals:
            pl = vis.get("pixel_layout") or {}
            key = (
                vis.get("name"),
                round(pl.get("pixel_x", 0), 1),
                round(pl.get("pixel_y", 0), 1),
                round(pl.get("pixel_width", 0), 1),
                round(pl.get("pixel_height", 0), 1),
            )
            if key in seen_vis:
                continue
            seen_vis.add(key)
            visuals_unique.append(vis)

        dashboards_data.append(
            {
                "dashboardName": db_name,
                "worksheets": ws_list_unique,
                "canvas": {"width": cw, "height": ch},
                "visuals": visuals_unique,
                "coordinateSystem": "tableau_0_100000",
            }
        )
